...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            # turns = strategy0(score0, score1)
            # if turns == 0:
            #     score0 = picky_piggy(score1) + score0
            # else:
            #     score0 = roll_dice(turns, dice) + score0
            score0 += (take_turn(strategy0(score0, score1), score1, dice, goal))
            score0 += hog_pile(score1, score0)
        else:
            score1 += (take_turn(strategy1(score1, score0), score0, dice, goal))
            score1 += hog_pile(score0, score1)
        #     turns = strategy1(score1, score0)
        #     if turns == 0:
        #         score1 = picky_piggy(score0) + score1
        #     else:
        #         score1 = roll_dice(turns, dice) + score1
        who = next_player(who)
        # END PROBLEM 5
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(*scores):
        ...
        if gain > running_high:
            string = str(gain) + " point(s)! That's a record gain for Player " + str(who) + "!"
            print(string)
...
def make_averaged(original_function, trials_count=1000):
    ...
    return try_and_average  # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for i in range(1, 11):
        ...
        arr.append(temp)
    return arr.index(highest) + 1
    # END PROBLEM 9python3 hog.py -r
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if picky_piggy_strategy(score, opponent_score, cutoff, num_rolls) == 0:
        return 0
    elif not hog_pile(score + picky_piggy(opponent_score), opponent_score) == 0:
        return 0
    return num_rolls
    # END PROBLEM 11
